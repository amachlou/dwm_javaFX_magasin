...
public class VenteDaoImpl extends AbstractDAO implements IVenteDAO{
	...
	public List<Vente> getAllVente() {
		...
		ResultSet rs;
		String query="select v.id_vente,CONCAT(c.prenom, ' ', c.nom) as name,v.id_client,v.date,v.total,p.total_paye,p.reste from vente v LEFT JOIN client c on c.id_client=v.id_client LEFT JOIN paiement p on (v.id_vente=p.id_vente and p.total_paye=(select p.total_paye from vente v,paiement p where v.id_vente=p.id_vente order by p.id_paiement DESC limit 1) and p.reste=(select p.reste from vente v,paiement p where v.id_vente=p.id_vente order by p.id_paiement DESC limit 1)) GROUP by v.id_vente";
		...
		try {
				...
				while(rs.next()) {
					...
					double reste=total-total_paye;
					list.add(new Vente(rs.getLong("id_vente"),rs.getString("name"),date.toLocalDate(),total,total_paye,reste,rs.getLong("id_client")));
				} 
